 ...
 *  - DMA coprocessor acceleration for bulk SPI transfers (>= 16 bytes)
 *  - Hardware floating-point unit (FPU) with explicit assembly instructions
 ...
 *  - Chip-select atomicity guaranteed on all code paths
...
/*--------------------------- FIFO Configuration ----------------------*/
#define LSM_FIFO_PACKET_SIZE   14U  /* 12 bytes data + 2 bytes padding */
...
#define IF_INC_BIT            (1U << 2)  /* Auto-increment enable */
...
#define DEG_TO_RAD_CONST      0.01745329252f  /* pi/180 */
#define RAD_TO_DEG_CONST      57.2957795131f  /* 180/pi */

/*--------------------------- Aligned Data Structures -----------------*/
/* Aligned frame buffer for DMA and FPU efficiency */
...
static lsm6_read_aligned latest_lsm[LSM_MAX_FIFO_SAMPLES] __attribute__((aligned(16)));
...
/*--------------------------- Coprocessor Handles ---------------------*/
static uint8_t lsm_dma_channel;
...
static inline uint8_t lsm_read_reg_with_burst_dma(lsm6_dev *dev, uint8_t reg, uint8_t *data, uint32_t len);
...
uint8_t LSM6_init(uint8_t spi_id)
{
    ...
    const uint8_t ctrl3_cfg = BDU_MASK | IF_INC_BIT;  /* Enable auto-increment */
...
/*----------------------------------------------------------------------
 *  @brief Read FIFO data using conditional DMA based on size
 *  @return 0 on success, non-zero error code
 *----------------------------------------------------------------------*/
uint8_t LSM6_readSensor(void)
{
    ...
    if (6U >= DMA_THRESHOLD_BYTES) {
        ...
        if (lsm_read_reg_with_burst_dma(&lsm_dev, REG_OUTX_L_XL, accel_data, 6U) != 0U) {
            return 4U;  /* CS deasserted in helper function */
        }
    } else {
...
/*======================================================================
 *  Private Helper Implementations - FPU Optimized with Atomic CS
 *======================================================================*/

/*----------------------------------------------------------------------
 *  @brief Write to device register via SPI
 *  @note CS is only asserted during the actual transaction and atomically protected
 *----------------------------------------------------------------------*/
static inline uint8_t lsm_write_reg(lsm6_dev *dev, uint8_t reg, const uint8_t *data, uint32_t len)
{
    ASSERT_NOT_NULL(dev);
 ...
 *  @note CS is only asserted during the actual transaction
 *  @note IF_INC bit must be set in CTRL3_C for auto-increment to work
 *----------------------------------------------------------------------*/
static inline uint8_t lsm_read_reg_with_burst(lsm6_dev *dev, uint8_t reg, uint8_t *data, uint32_t len)
{
    ASSERT_NOT_NULL(dev);
...
/*----------------------------------------------------------------------
 *  @brief Read from device register with DMA burst transfer
 *  @note Only uses DMA if length >= DMA_THRESHOLD_BYTES
 *  @note CS is only asserted during the actual SPI transaction
 *  @note IF_INC bit must be set in CTRL3_C for auto-increment to work
 *----------------------------------------------------------------------*/
static inline uint8_t lsm_read_reg_with_burst_dma(lsm6_dev *dev, uint8_t reg, uint8_t *data, uint32_t len)
{
    ...
    /* Use blocking SPI for small transfers (avoids DMA overhead) */
    if (len < DMA_THRESHOLD_BYTES) {
        return lsm_read_reg_with_burst(dev, reg, data, len);
    }
    ...
    /* Critical section: ensure CS atomicity and start DMA */
    irq_disable();
    gpio_put(dev->cs_pin, false);  /* Assert CS */

    /* Start DMA transfer */
    if (dma_start_transfer(lsm_dma_channel, (const void*)tx_buf, data, len + 1U, 0U) != 0U) {
        gpio_put(dev->cs_pin, true);  /* Release CS on error */
        irq_enable();
 ...
 *  @note CS handled atomically during register writes
 *----------------------------------------------------------------------*/
static void lsm_flush_fifo(lsm6_dev *dev)
{
    ASSERT_NOT_NULL(dev);
    ...
    const uint32_t flush_delay = 1000U;
    for (volatile uint32_t i = 0; i < flush_delay; i++);
...
static inline void lsm_configure_cs(void)
{
    gpio_set_dir(LSM_SPI_CS_PIN, true);  /* Output */
    gpio_put(LSM_SPI_CS_PIN, true);      /* Inactive high (CS is active low) */
}
